...
class mtcnn():
    def __init__(self):
        self.Pnet= creat_Rnet('model_data/pnet.h5')
        self.Rnet = creat_Rnet('model_data/rnet.h5')
        self.Onet = creat_Rnet('model_data/onet.h5')
    def detectFace(self,img,threshold):
        copy_img=(img.copy()-127.5)/127.5
        origin_h,origin_w,_=copy_img.shape
        scales =utils.calculatScale(img)
        out =[]
        for scale in scales:
        ...
        for i in range(image_num):
            ...
            rectangle= utils.detect_face_12net(cls_prob,roi,out_side,1/scale[i],origin_w,origin_h,threshold[0])
