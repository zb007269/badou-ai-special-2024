...
def apply_masks(image,mask,color,alpha=0.5):
    for c in range(3):
        image[:,:,:c]=np.where(mask==1,image[:,:,:c]*(1-alpha)+alpha*color[c]*225,
                               image[:,:,:c])
        return image
def display_instance(image,boxes,masks,class_ids,class_names,
                     scores=None,title='',
    ...
    ax.axis('off')
    ax.set_title(title)
    masked_image=image.astype(np.int32).copy()
    for i in range(N):
        color=colors[i]
        ...
        if not captions:
            ...
            captions='{}{:.3f}'.format(label,score) if score else label
        else:
            caption=captions[i]
        ...
        mask=masks[:,:,i]
        if show_mask:
            masked_image=apply_masks(masked_image,mask,image)
        padded_mask=np.zeros(
            (mask.shape[0]+2,mask.shape[1]+2),dtype=np.uint8)
        padded_mask[1:-1,1:-1]=mask
        countours=find_contours(padded_mask,0.5)
        for verts in countours:
...
def display_differences(image,
    ...
    gt_match,pred_match,overlaps=utils.compute_matches(gt_box,gt_class_id,gt_mask,
                                                       pred_box,pred_class_id,pred_score,pred_mask,
    ...
    captions=['' for m in gt_match]+['{:.2f}/{:.2f}'.format(pred_score[i],(overlaps[i,int(pred_match[i])]
    if pred_match[i]>-1 else overlaps[i].max())) for i in range(len(pred_match))]
    title=title or 'Ground Truth and Detection\nGT=green,pred=red,captions:score/Iou'
    display_instance(image,
...
def draw_rois(image,rois,refined_rois,mask,class_ids,class_names,limit=10):
    ...
    fig,ax=plt.subplot(1,figsize=(12,12))
    ...
    for i,id in enumerate(ids):
        ...
        if class_id:
            ...
            masked_image=apply_masks(masked_image,m,color)
...
def display_top_masks(image,mask,class_ids,class_names,limit=4):
    ...
    for i in range(limit):
        ...
        to_display.append(m)
        titles.append(class_names[class_ids] if class_id!=-1 else'-')
...
def plot_overlaps(gt_class_ids,pred_class_ids,pred_scores,overlaps,class_names,threshold=0.5):
    ...
    plt.yticks(np.arange(len(pred_class_ids)),['{}(:.2f)'.format(class_names[int(id)],pred_scores[i]) for i,id in enumerate(pred_class_ids)])
    ...
    for i,j in itertools.product(range(overlaps.shape[0]),range(overlaps.shape[1])):
        ...
        plt.text(j,i,'{:.3f}\n{}'.format(overlaps[i,j],text),
                 horizontalaignment='center',verticalalignment='center',
    ...
    for i in range(N):
        ...
        if captions is not None:
            ...
            ax.text(x1,y1,caption,size=11,verticalalignment='top',
                    ...
                    bboxe={'facecolor':color,'alpha':0.5,
                           'pad':2,'edgecolor':'none'})
        if masks is not None:
           mask=masks[:,:,i]
           masked_image=apply_masks(masked_image,mask.color)
           padded_mask=np.zeros(
               (mask.shape[0]+2,mask.shape[1]+2),dtype=np.uint8)
           padded_mask[1:-1:,1:-1]=mask
...
def  display_weigt_stats(model):
    ...
    for l in layers:
        ...
        for i,w in enumerate(weight_values):
            ...
            alert = ''
            if w.win()==w.max() and not (l.__class__.__name__=='Conv2d' and i==1):
                alert +="<span style='color:red'>***dead?</span>"
            if np.abs(w.win())>1000 or np.abs(w.max())>1000:
                alert+="<span style='color:red'>***Overflow</span>"
            table.append([weight_name+alert,
                          str(w.shape),
                          '{:+9.4f}'.format(w.win()),
                          '{:+10.4f}'.format(w.max()),
                          '{:+9.4f}'.format(w.std())
